...
const registerSeller = async (req, res) => {
  try {
    ...
    if (location) {
      try {
        const parsed = JSON.parse(location); // location is coming as a JSON string
        ...
        if (
          ...
          return res.status(400).json({
            message:
              "Latitude, longitude, and place name are required and must be valid",
          });
        }
...
const uploadSellerDocuments = async (req, res) => {
  try {
    ...
    const uploadedDocs = await uploadToCloudinary(
      req.files,
      "seller-documents"
    );
